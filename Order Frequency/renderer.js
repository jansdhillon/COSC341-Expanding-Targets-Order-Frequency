...
// var randomFrequency = [0,0,0,0,0,0];

// var clickFrequency = [0,0,0,0,0,0];
...
var buttons = parent.getElementsByTagName("img");
...
function save()
{
  ...
  fs.writeFile( path.resolve(fileDir, "OrderFrequency"+trial+".csv"), dataLog, (err)=> {
    if (err) alert(err);
...
var timedClick = function()
{
  ...
  // randomFrequency[targetIndex]++;
  // while (randomFrequency[targetIndex] > numIcons){
  ...
  for(var i=0; i < buttons.length; i++)
  {
    buttons[i].onclick = function()
    {
      ...
      // var iconClicked = this.id[1];
      // clickFrequency[iconClicked]++;
      // iconsCalled[targetIndex] += 1;
      clicks++;
     

      
      // prevClick = iconClicked;
      var volume = this.dataset.volume; // INCLUDE THIS
      var volumeclicked = volume/20;
      iconsCalled[volumeclicked]++;
      dataLog = dataLog +clicks+", "+targetVolume+", "+  volume+", "+totalTime+ "\n";
      display.innerHTML = "Current Volume: " + volume;
      music.volume = volume /100;
      var newOrder = Array.from(Array(iconsCalled.length).keys()).sort(
        function(a,b){
          if (iconsCalled[a] > iconsCalled[b]){ 
            return -1;
          } else if (iconsCalled[b] > iconsCalled[a]){
             return 0;   
          } else if (iconsCalled[a] == iconsCalled[b]){
             return 0;
          }
        });
...
var newOrder = Array.from(Array(iconsCalled.length).keys()).sort(
      ...
      newOrder.forEach( argIndex => parent.appendChild(document.getElementById("button-"+argIndex)));
      return;
      // frequencyDisplay.innerHTML = "Frequencies: " + iconsCalled[0] + ", " + iconsCalled[1] + ", " + 
      // iconsCalled[2] + ", " + iconsCalled[3] + ", " + iconsCalled[4] + ".   New Order: " + newOrder[0] + ", " + newOrder[1] + ", " + newOrder[2] + ", " + newOrder[3] + ", " + newOrder[4];
}
